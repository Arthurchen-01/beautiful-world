...
**版本：v1.5.0** ｜ Windows 单文件 exe ｜ 内置 OCR 识别引擎 ｜ **网页控制台**
...
| 扫描 | 参数 → 启停 → **实时日志** → 结果预览 → 下载 |
| 历史 | 每批次时间、退出码、结果行数、预览、下载 |
| 代理池 | 编辑 → 一键体检（可用数 / 唯一 IP / **国内出口告警** / 逐条延迟） |
| 设置 | 账号文件编辑 / 面板账号增删 / 改密码 |

**安全设计**：

- **只监听 `127.0.0.1`** —— 公网暴露交给 Cloudflare Tunnel / 反向代理
- 团队多人账号，PBKDF2-SHA256 存哈希，HttpOnly Cookie 会话
- 首次启动生成管理员密码，写在 `webui\初始密码.txt`（登录后改掉并删除）

**推荐接 Cloudflare Tunnel**（服务器**零入站端口**、真实 IP 完全隐藏、免费 HTTPS、
可加邮箱验证第二层认证）—— 步骤见
[`docs/网页面板_Cloudflare接入.md`](docs/网页面板_Cloudflare接入.md)。

> **为什么不直接用 IP**：面板要传几万个账号的明文密码，而**纯 IP 拿不到
> Let's Encrypt 证书**，只能明文 HTTP。而且这台机器的 IP 要去撞完美世界，
> 不该被摆到全世界的扫描器面前。
